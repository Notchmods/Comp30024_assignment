...
def evaluation(state: GameState, color: PlayerColor):
    ...
    POSITION_WEIGHT  =  1.0
    THREAT_PENALTY   = 15.0  # match material weight
    ...
    for coord, (piece_color, height) in state.board.items():
        ...
        if piece_color == color:
            ...
            # your pieces: check if threatened by opponent
            is_threatened = False
            ...
            # opponent pieces: check if threatened by you
            is_opp_threatened = False
            for d in (Direction.Up, Direction.Down, Direction.Left, Direction.Right):
                try:
                    adj_coord = coord + d
                    adj_piece = state.board.get(adj_coord)
                    # only break when a genuine threat is confirmed
            ...
            if is_opp_threatened:
                opp_threat_score -= (height * THREAT_PENALTY)

    # elimination pressure increases as game goes on
